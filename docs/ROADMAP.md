...
| 30.5 | ~~Cascade routing~~ → отдельный slice | ⏭️ | Multi-hop через keygen-issued inter-node secrets (был частью 30, выделен).
...
| 31.1 | **Multi-protocol recipe bundles** | ⏭️ | `recipe.profiles[]` (REALITY+xhttp + Hysteria2+Salamander одной кнопкой), auto-binding fan-out, auto-host failover priority. Поверх slice 31. |
