...
| 24c | **Xray defaults uplift + transports/subprotocols** | 🟡 parts 1+2+3a done | ✅ **Part 1**: per-user stats — `stats:{}`, `policy.levels.0.statsUserUplink/Downlink`, dedicated `api-in` (dokodemo-door 127.0.0.1:8080), `GetStats` через `xray api statsquery -reset` + parser, soft-fail. ✅ **Part 2**: `httpupgrade` + `kcp` transports + routing defaults (sniffing, dns-out, blackhole + BLOCK rules для bittorrent/port:25, sockopt BBR + tcpFastOpen). ✅ **Part 3a**: Trojan subprotocol — `subprotocol: 'vless' \| 'trojan'` field в XrayConfigSchema, новый `buildTrojanRealityUri` (`trojan://password@host:port?...`), субscription branch'ит на subprotocol; node renderConfig переключает inbound `protocol` + `clients` shape (vless: id+flow+decryption / trojan: password); password переиспользует `user.xrayUuid`; frontend Select. ⏭️ **Part 3b TODO**: Shadowsocks (SS2022) — отдельный коммит (мульти-юзер модель отличается, per-user keys + cipher selection). |
