...
| 24d | **Shadowsocks (SS2022) как top-level protocol** | ✅ done (code) | Новый `protocol: 'shadowsocks'` в panel discriminated union; `ShadowsocksConfigSchema` с cipher selector (3 SS2022 + 3 legacy AEAD); SIP002 URI builder (`ss://base64url(method:password)@host:port#name`, no padding). Per-user password переиспользует `user.xrayUuid` — без новой колонки в users. Новый Go-пакет `internal/core/shadowsocks/` с отдельным ShadowsocksAdapter, который шарит xray binary с XrayAdapter (SS2022 multi-user встроен в xray-core). Render config содержит full stats wiring (api-in на 127.0.0.1:8081 — на 1 выше xray'ского 8080), routing defaults (sniffing/dns-out/blackhole + BLOCK rules), sockopt-BBR. Adapter регается когда `XRAY_BINARY` set, deferred-method до первого ApplyInbound. Frontend: protocol Select option, Cipher Select section с подсказкой про переиспользование UUID. Singbox + Clash native format support — TODO в follow-up (sub-генератор пока эмитит plain SS URI; native shadowsocks outbound shapes в singbox/clash формате — отдельный коммит). |
