...
| 31 | **Transport Recipes Phase 2: Test-Connect + Multi-protocol bundles** | ⏭️ | Поверх slice 30. (a) **Test-Connect button** в ProfileFormModal: panel-side спин ап эфемерного xray-test-клиента → попытка handshake к inbound → 🟢/🔴 результат за 2 сек. (b) **Multi-protocol recipes**: `recipe.profiles[]` вместо одного profile — recipe «Max DPI resistance for RU mobile» создаёт 2 профиля (REALITY+xhttp + Hysteria2+Salamander) + auto-bindings + auto-hosts с failover priority. Subscription URL получает 2-3 строки, клиент Mihomo/Singbox `url-test` сам выбирает. |
