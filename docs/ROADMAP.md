...
| 24c | **Xray defaults uplift + transports/subprotocols** | 🟡 parts 1+2 done | ✅ **Part 1**: per-user stats — `stats:{}`, `policy.levels.0.statsUserUplink/Downlink`, dedicated `api-in` (dokodemo-door 127.0.0.1:8080), `GetStats` через `xray api statsquery -reset` + parser, soft-fail. ✅ **Part 2**: `httpupgrade` + `kcp` transports добавлены в network enum и render-branch. Routing defaults — sniffing (http/tls/quic) на vless inbound, `dns-out` outbound + DNS hijack rule, `blackhole` outbound + BLOCK rules (bittorrent, port:25), sockopt BBR + tcpFastOpen на direct outbound. Frontend Select получил httpupgrade/kcp, path/host поля показываются и для httpupgrade. ⏭️ **Part 3 TODO**: Trojan + Shadowsocks subprotocols (отдельный коммит — новый Zod schema branch + URI builder + render path). |
