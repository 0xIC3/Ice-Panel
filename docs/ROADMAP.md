...
| 24d | **Shadowsocks (SS2022) как top-level protocol** | ⏭️ | Новый `protocol: 'shadowsocks'` в panel discriminated union, `ShadowsocksConfigSchema` (cipher selector: chacha20-ietf-poly1305 / aes-256-gcm / 2022-blake3-aes-256-gcm), per-user password (или derive из xrayUuid через HKDF). Новый ShadowsocksAdapter (или extend XrayAdapter управлять multi-inbound). URI builder `ss://base64(method:password)@host:port#name`. Singbox + Clash format support. Frontend section в InboundFormModal. Не subprotocol of xray — SS не TLS/REALITY-based. |
