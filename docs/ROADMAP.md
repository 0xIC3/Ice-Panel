...
| 30 | **Hosts abstraction (full)** + **Cascade routing** | ⏭️ | `hosts` table (one binding → many hosts) с per-host overrides: `sniOverride`, `hostHeader`, `pathOverride`, `securityLayer` (default/tls/none), `alpn[]` (h3/h2/http/1.1 combos), `fingerprint`, `disableForFormats[]`. Subscription generator итерирует bindings × hosts → один binding отдаёт N URL'ов. Разблокирует ws-через-CDN (hosts с `securityLayer: tls` поверх REALITY-inbound), per-region SNI tuning, A/B-testing транспортов. UI: `/hosts` page или drawer внутри ProfileCard с drag&drop priority. |
