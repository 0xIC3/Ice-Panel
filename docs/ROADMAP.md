...
| 27c | **Transport Recipes + Live Validator + UX polish** (Phase 1 differentiator) | ✅ done (2026-05-08) | Frontend-only слой поверх `ProfileFormModal` который превращает sea-of-fields-форму в **«Что хочешь достичь?»**-style configurator. (1) **`apps/panel-frontend/src/lib/recipes.ts`**: библиотека из 11 pre-validated рецептов keyed по протоколу — REALITY+Vision raw / REALITY+xhttp / Trojan+REALITY / Hysteria clean / Hysteria+Salamander / AWG default / AWG Iran-tuned / Naive / SS-2022 / MTProto / Mieru. Каждый Recipe: emoji + name + 1-line description + DPI★/Speed★ ratings + multiline details (tooltip) + `apply` overrides + optional notes (банеры). (2) **`RecipePicker.tsx`**: SimpleGrid карточки выше protocol-specific конфигурации, click → `form.setValues` merge + alert «применён». (3) **Auto-fill missing crypto** на recipe pick: для xray генерится REALITY keypair (через `/api/profiles/generate-keypair`) + 6 случайных 16-hex shortIds; для AmneziaWG — server keypair. Один клик = форма ready-to-save. (4) **`validateXrayConfig()` live-validator**: inline-alerts красного/жёлтого/синего на каждом изменении полей — `🔴 REALITY+ws не загрузится`, `🟡 Trojan не использует flow`, `🔵 Без Vision на raw теряется ~30% throughput`. Защита от багов до save. (5) **AWG H1-H4 UX overhaul**: убран yellow-alert «запусти `shuf -i 5-2147483647 -n 4`», вместо него — `Re-roll`-button + `randomAwgHeaders()` helper (5..int32, попарно уникальны), auto-roll при выборе TSPU/Mobile preset, live-validator на дубликаты. (6) **Form layout polish**: xray-блок переписан в 2-column grid (dest↔serverNames, shortIds↔fingerprint, publicKey↔subprotocol, flow↔network), Имя/Протокол row top-aligned, immutability protocol-hint в edit-mode, mirror Zod name-regex client-side (`/^[a-zA-Z0-9._-]+$/`). (7) **DeployProfileModal protocol-mismatch warning**: NodeRow получает `profileProtocol` prop, рисует жёлтый border + `⚠ XRAY` badge с tooltip «binding создастся, но клиенты не подключатся пока install-node заново с --protocol hysteria», если node.protocol ≠ profile.protocol. **Differentiator над Remnawave/Marzban**: те бросают админа в alphabetic-list raw fields; у нас Recipe Library + Live Validation + Auto-fill = onboarding-quality UX. Phase 2 (Hosts abstraction + Test-Connect button + Multi-protocol bundles) — slice 30+31. |
| 27b | **Deploy-to-nodes UI + первый VPS cycle #3 fix-up** | ✅ done (2026-05-08) | `DeployProfileModal` на `/profiles` — главная CTA на карточке профиля «Развернуть на нодах», открывает чек-лист всех нод (с status-badge), на сохранение diff'ит и вызывает `createBinding`/`deleteBinding` (порт берётся из `profile.config.port` по умолчанию, per-node override'ы пока через API). Закрыт TODO из slice 27 «bindings создаются через raw API». Параллельно — Slice 27 cleanup hangover: `/api/inbounds/generate-keypair` переехал в `/api/profiles/generate-keypair` (модуль inbounds снят с регистрации в slice 27). Cron healthcheck semantics fix: `degraded` (агент жив, но протокол-core не запущен — норма для свежей ноды без bindings) больше не флипает status в `unreachable`, остаётся `online` с пометкой в `lastStatusMessage`. install-panel.sh: `PUBLIC_URL` теперь генерится в `.env.production` (раньше undici-валидация Zod падала на пустой строке после docker-compose substitution). Frontend: sidebar порядок Профили → Squads → Nodes; Settings → API tokens row получил кнопку «копировать ID». |
| 27 | **Profile + ProfileNodeBinding refactor** (Remnawave-style) | ✅ done (2026-05-08) | Inbound разделён на 3 сущности: `Profile` (логический шаблон, имя глобально-уникально), `ProfileNodeBinding` (m:n деплой на ноду + per-node overrides — port, ACME домен, AmneziaWG privkey, SS server PSK, MTProto-derived secret), `GroupProfile` (squad ACL на уровне профиля). Subscription generator переписан под `user→squads→groupProfiles→bindings`. AmneziawgPeer мигрирован на profileId (один user → один WG IP, шарится по нодам одного профиля). Inbound-sync queue резолвит `profile.config + binding.overrides` shallow merge перед пушем (wire-формат к ноде не изменился). 2 миграции с 1:1 backfill (UUID inbound'а переиспользуется как profile.id). Frontend: `/profiles` card-grid + `ProfileFormModal` (per-protocol config editor), `SquadFormModal` группирует профили по протоколу. `/inbounds` редиректит на `/profiles`. Test suite: 24 файла / 221 тест зелёные. Старые таблицы `inbounds`/`group_inbounds` оставлены как dead weight для отката — финальный drop отдельной миграцией после проверки на VPS. BindingsPage UI закрыт в slice 27b. Multi-node management UI (регионы, capacity per node, sticky user→node) был исходным замыслом — переехал на slice 27.5/28. |
| 27.5 | **Multi-node management UI** | ✅ done (2026-05-10) | Новая таблица `regions(id, name UNIQUE, code UNIQUE)`, FK `Node.regionId` (`ON DELETE SET NULL` — удаление региона обнуляет ноды, не каскадит), `Node.maxUsers Int?` capacity hint, `User.preferredNodeId Uuid?` sticky-affinity (стартовый bias для slice 28+29 url-test). Backend: `regionsRoutes` CRUD + расширение `nodes.schemas.ts` Zod (`regionId`/`maxUsers` create/update/list-filter), `nodes.repository` переключён на `Prisma.NodeUncheckedCreateInput`/UpdateInput для flat FK assignment. Frontend: `RegionsCard` в SettingsPage с inline CRUD; `NodeEditModal` получает Region Select (clearable, label `code · name`) + maxUsers NumberInput в Параметрах; `NodeCard` рисует cyan-chip региона рядом с именем + capacity Progress bar (color-graded red>100% / yellow>85% / teal) когда `maxUsers > 0`; `NodesPage` — region filter chips ниже хедера (скрыто пока ни одного региона нет, чтобы не захламлять fresh panel). `approxUsers` в capacity bar — placeholder из `inboundCount`, ground-truth per-user counter лендится с slice 28. 228/228 backend tests green, frontend tsc + build clean. |
| 27.6 | **Multi-protocol nodes** | ⏭️ | Сейчас одна нода = один протокол (install-node ставит ОДИН бинарь). Schema migration `node.protocol: string → node.installedProtocols: string[]`, install-node принимает comma-list (`--protocol xray,hysteria`), agent уже multi-adapter (готов), DeployModal protocol-mismatch warning расслабляется (красный только при отсутствии любого match'а). Экономия на VPS: 1 нода может держать xray+hy2+awg одновременно вместо 3 отдельных машин. |
| 28 | **Server-side smart node selection** | ✅ skeleton (2026-05-11) | `apps/panel-backend/src/modules/subscription/node-selection.ts` — `lookupClientCountry(ip, {cfCountry})` (Redis-cache 60s, CF-IPCountry backend, null когда XX/missing) + `rankNodesForUser(nodes, country, limit?)` (score = regionMatch×100 + utilization×50, ordered desc). Callers подключают по желанию. MaxMind GeoLite2 DB и `user.preferAllNodes` opt-out отложены до slice 28 follow-up. |
| 29 | **Subscription `url-test` groups** | ✅ done (2026-05-11) | Clash формат уже emit'ил `url-test` (default proxy-group). Sing-box formatter теперь принимает `SingboxBuildOpts.bundle = 'selector' \| 'url-test'`, defaults 'selector'. Subscription route читает `?bundle=` query, проксирует в singbox formatter. `url-test` form: `type: urltest`, probe `https://www.gstatic.com/generate_204`, interval 300s, tolerance 50ms. Xray `burstObservatory + balancer` отложено (xrayjson formatter still emits flat outbounds). |
| 30 | **Hosts abstraction** | ✅ done (2026-05-10) | `hosts` table FK→`profile_node_bindings` (cascade) с per-host overrides: `addressOverride`/`portOverride`, `sniOverride`, `hostHeaderOverride`, `pathOverride`, `fingerprintOverride`, `alpn[]`, `allowInsecure`, `securityLayer` (default/tls/none), `disableForFormats[]`, `priority`, `enabled`, `remark`. Backfill migration вставляет 1 Default host на каждый существующий binding → подписка работает идентично до. `createBinding` на бэке делает `ensureDefaultHost(bindingId)` → новые bindings тоже стартуют с одним host. Subscription generator переписан на bindings × hosts с shallow-merge override'ов (NULL → fallback в profile.config / binding). Sub-routes делает format-gating через `endpoint.disableForFormats` перед инвокацией formatter'а. SubscriptionEndpointBase получил optional `hostId`/`hostRemark`/`alpn`/`allowInsecure`/`securityLayer`/`disableForFormats` для slice 30.1 (light up alpn/insecure emission в URI builders). Frontend: `HostsManager` компонент (CRUD + ↑↓ reorder через `/api/hosts/reorder`) встроен в NodeEditModal под каждый binding row. Per-protocol form gating: AmneziaWG скрыт (single-endpoint), Xray показывает sni/path/host/fp/ALPN, остальные — только address/port/security. Реальный drag&drop отложен в slice 31 (стрелок ↑↓ хватает для 2-4 host'ов). 228/228 tests green. **Что разблокирует:** REALITY+CDN front (host со `securityLayer:tls` поверх REALITY-inbound), multi-SNI fronting, ALPN A/B, fingerprint diversity, per-format URL gating. |
| 30 (full) | **Hosts abstraction (полная)** | ⏸️ **deliberately deferred (2026-05-11)** | Phase 1 (single host-row per binding с overrides) уже работает + 30.1 emit'ит alpn/securityLayer в URI и formatter. **Phase 2 (отдельный `inbound_hosts` table, one inbound → N client-facing hosts)** оставлен на запрос: текущий single-host-per-binding с per-host overrides покрывает все наблюдаемые use cases (REALITY + CDN front, multi-SNI). Перенесение в отдельную таблицу — миграция данных + изменение subscription pipeline без видимого UX win для MVP. Когда: при появлении конкретного запроса от community deployment'а, требующего >1 host на binding |
| 30.5 | ~~Cascade routing~~ | ⏸️ **deliberately deferred (2026-05-11)** | "Архитектурно опасный" по самой ROADMAP. Multi-hop через `node_peer_secrets` + keygen-issued inter-node creds, propagation через mTLS, fan-out at rebuild — это отдельный quarterly project. **Не браться без конкретного use case** — MVP не нуждается в этом, добавляется по требованию.
| 30.1 | **Hosts emission Phase 2** | ✅ partial (2026-05-11) | URI: `buildVlessRealityUri` + `buildTrojanRealityUri` теперь принимают `alpn?: string[]`, `allowInsecure?: bool`, `securityLayer?: 'default'\|'tls'\|'none'`. subscription.service передаёт `hostMeta` в оба builder. URI emit: `alpn=h2,http/1.1`, `allowInsecure=1`, `security=tls\|none\|reality`. ALPN в clash/singbox formatters + drag&drop UI reorder — ТОDO (формат-уровень overrides уже работают через `host.alpn` field в endpoint metadata). |
| i18n-Phase-2 | **i18n: остальные страницы (ru/en)** | ✅ done (2026-05-10) | Расширил `ru.ts`/`en.ts` namespaces: `common` (+actions/save/saveError/etc), `dashboard`, `profiles`, `users`, `squads`, `settings` (incl. subscription metadata), `hosts`. Подключил `useTranslation()` и заменил хардкод-строки в ProfilesPage / UsersPage / DashboardPage (chrome) / SettingsPage / HostsManager. Заголовки + основные кнопки + notify-сообщения + delete-confirms + table headers. Глубокие labels форм (UserFormModal, ProfileFormModal, NodeFormModal) **остались на russian-only** — они высоко специфичны и не критичны для bilingual demo; можно докрыть при необходимости отдельным slice'ом. Frontend type-check + build clean. |
| 31 | **Test-Connect button** | ✅ done (2026-05-10) | `POST /api/profiles/:id/test-connect` резолвит все enabled bindings × hosts профиля и для каждого пускает outbound probe из panel-контейнера: TLS handshake (xray REALITY / naive — рабочий signal даже для borrowed-cert REALITY, потому что `rejectUnauthorized: false` + читаем peer CN), TCP probe (shadowsocks / mtproto / hysteria / amneziawg / mieru — UDP-протоколы помечены `notes: 'TCP reachability only'` чтобы зелёный tick не врал). 5s timeout per probe, все probes parallel → response landing ≤ ~6s. Frontend: `TestConnectModal` с кнопкой «Test connect» в ProfileCard menu, auto-fires при открытии, рендерит rows с `endpoint:port`, SNI, peer cert CN, latency-pill, error-string и yellow-warning для UDP. Re-run button. **Differentiator над Remnawave/Marzban**: вместо ssh+curl/openssl на ноде admin делает 1 click и видит зелёный/красный per-host. **Multi-protocol recipe-bundles** отложены — без них одиночные recipes уже покрывают типичный flow; bundles требуют схема для recipe.profiles[] + subscription multi-emit, отдельный slice 31.1. 228/228 backend tests green. |
| 31.1 | **Multi-protocol recipe bundles** | ⏸️ **deliberately deferred (2026-05-11)** | UX win поверх slice 31. Зависит от 30/31. Одиночные recipes (Test-Connect slice 31) уже покрывают типичный setup-flow для MVP. Реализация ~2 дня когда придёт время |
| 30.5 | ~~Cascade routing~~ | ⏭️ | Multi-hop через keygen-issued inter-node secrets (NOT fake-user trick) — выделено в отдельный slice, был частью 30. |
| 31 | **Cross-protocol cascade** | ⏸️ **deliberately deferred (2026-05-11)** | Зависит от slice 30.5 (cascade routing), который сам deferred. Hysteria↔Xray bridging — niche feature, никогда не запрашивался. Реализация = ~3 дня после 30.5. Откроем когда появится конкретный use case |
| 32 | **Telegram bot + Webhook notifications** | ✅ partial (2026-05-11) | Не bot (нет grammy → нет команд от user'а), а **admin alerts via Bot API** через `apps/panel-backend/src/lib/telegram-notify.ts`. Конфиг — `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` (no-op when unset). События: `auth.login_ok` / `auth.lockout` (auth.routes), `honeypot.first-hit` / `geo-block.denial` (security-gate), `node.status-flip ✅/🔴` (nodes.cron), `user.expired ⏳` / `user.limited 📊` (users.events). Webhook framework + per-event chat_id routing + bot-level commands (user-facing) — TODO. |
| 33 | **Prometheus metrics + Grafana dashboards** | ✅ partial (2026-05-11) | `prom-client` registry в `apps/panel-backend/src/lib/metrics.ts` + `/metrics` route (gated через requireAuth — Prom scrape с `icp_*` Bearer). Default metrics (Node.js process — CPU/mem/GC/eventloop) + HTTP histogram (method/route/status, low-card route label из routeOptions.url) + 6 custom: `ice_panel_login_attempts_total{result}`, `ice_panel_subscription_requests_total{format}`, `ice_panel_inbound_sync_jobs_total{result}`, `ice_panel_honeypot_hits_total`, `ice_panel_geo_block_denials_total`, gauges `ice_panel_nodes{status}` + `ice_panel_users{status}` (refresh-loop 30s через `metrics-refresh.ts`). Grafana dashboards — TODO (нужно для full slice closure). |
| 34 | **Backup / restore CLI** | ✅ done (2026-05-10) | Два bash-скрипта в `scripts/`: `ice-panel-backup.sh` (pg_dump через `docker exec` postgres-контейнер + redis BGSAVE→docker cp `/data/dump.rdb` + копия host `.env.production` + `manifest.json` → tar.gz, опционально AES-256-CBC через `openssl enc -pbkdf2 -iter 200000` с `--password`) и `ice-panel-restore.sh` (decrypt → unpack → manifest preview → confirm prompt → stop panel-backend+frontend → `DROP SCHEMA CASCADE` + psql dump → stop redis + replace dump.rdb + flush AOF leftovers → restart). Восстановление **не перезаписывает .env.production** — если JWT_SECRET сменился между хостами, blind-replace сломал бы свежевосстановленный DB; admin merge'ит вручную. CA private key лежит внутри pg_dump → восстановленная панель не требует переустановки нод (mTLS сохраняется). docs/deploy/backup.md с cron-примером + recovery scenarios. Bash syntax-check passed; интеграционная проверка отложена до VPS-cycle. |
| 35 | **Security hardening** | ✅ partial (2026-05-11) | `pnpm audit --prod --audit-level high` gate в CI job `panel`; per-route rate-limits закрыты slice S7 (cycle #5); CSP refinement + fuzzing — отложено до публичного релиза. |
| 36 | **CI/CD via GitHub Actions** | ✅ done (2026-05-11) | `.github/workflows/ci.yml` — два job: `panel` (pnpm install → prisma generate+migrate против live Postgres+Redis service-containers → tsc backend+frontend → vitest backend) и `node-agent` (Go 1.22 → build + vet + test). Concurrency-cancel при rebase, 15-min timeout. Docker build + ghcr publish — отложено до публичного релиза. |
...
| Название | Status | Что вводим |
|---|---|---|
| **API tokens (UI + CRUD + Bearer auth)** | ✅ done (2026-05-08) | Backend: `/api/api-tokens` GET/POST/DELETE на ApiToken-модели. Plaintext выдаётся **раз** (`icp_<base64url-32b>`), хранится только SHA-256 hash. Reveal-modal с copy-once UX. **Bearer-auth integration в `requireAuth`** — hook сначала пробует `Bearer icp_*` (lookup по SHA-256, фильтр по `apiToken.tokenHash`), при попадании ставит `request.admin` + `request.apiToken`, fire-and-forget апдейт `lastUsedAt`; иначе fallback на JWT. `/api/api-tokens` защищён вторым хуком `blockApiTokenAccess` (403 если запрос пришёл от API-token — leaked токен не должен делать новые). 7 тестов покрывают create/list/delete/Bearer-auth/lastUsedAt/anti-escalation. Frontend: `/settings` с тремя секциями. **TODO future:** scopes-enforcement (сейчас scopes хранятся, но игнорируются — токены = full admin). |
...
| **Subscription headers + Announce template** (S1) | ✅ done (2026-05-10) | На `/sub/:token` теперь идут пять заголовков: `Profile-Title` (base64-encoded UTF-8, fallback на brandName), `Profile-Update-Interval` (часы, default 24), `Subscription-Userinfo: upload=0; download=N; total=M; expire=T` (генерится из user.traffic + user.expireAt — клиент рисует quota-gauge), `Support-URL`, `Announce` (base64-encoded, рендерится из template-строки с `{{TRAFFIC_LEFT}}`/`{{DAYS_LEFT}}`/`{{SUPPORT_URL}}`). Хранится в существующей `app_settings` таблице — четыре новых ключа: `subscriptionProfileTitle`/`subscriptionUpdateIntervalHours`/`subscriptionSupportUrl`/`subscriptionAnnounceTemplate`. Helper `getSubscriptionSettings()` и `formatBytes()` в `settings.service.ts`. Frontend: карточка «Метаданные подписки» в SettingsPage с TextInput'ами и Textarea для template'а. NULL/empty в любом поле → соответствующий header не выдаётся. 228/228 backend tests green. |
| **HWID enforcement на /sub/** (S2) | ✅ done (2026-05-10) | Новая таблица `hwid_user_devices` (FK→users cascade, UNIQUE `(user_id, hwid)`, поля firstSeenAt/lastSeenAt/label). На `/sub/:token` middleware читает `x-hwid` header (≤255 chars), upsert'ит row, если новое устройство И `count >= user.hwidDeviceLimit` → 403 `HWID_LIMIT` + `X-Hwid-Active: N/M` header. При allowed/disabled тоже шлём `X-Hwid-Active: N/M` (или `N/unlimited` ASCII-safe — '∞' падает в http2 на ISO-8859-1 валидации). Lazy population: row только когда клиент реально шлёт header. Trust model = "семейный родитель", spoof header = bypass (acceptable). Admin routes `GET /api/users/:userId/hwid-devices` + `DELETE /api/hwid-devices/:id` для просмотра и сброса слотов. Frontend: UserFormModal в edit-режиме рендерит `UserDevicesPanel` с list устройств (truncated hwid + last-seen + delete-button). External-squad fallback / global-default отложены — per-user limit достаточно для v1. 228/228 tests green. |
